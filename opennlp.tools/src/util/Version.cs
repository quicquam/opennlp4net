 ...
 */
	  ...
	  {
		return Convert.ToString(Major) + "." + Convert.ToString(Minor) + "." + Convert.ToString(Revision) + (Snapshot ? SNAPSHOT_MARKER : "");
	  }
	  ...
	  public override bool Equals(object o)
	  {
		...
		else if (o is Version)
		{
		  ...
		  return Major == version.Major && Minor == version.Minor && Revision == version.Revision && Snapshot == version.Snapshot;
		}
		else
		{
		  return false;
	  ...
	  public static Version parse(string version)
	  {
		...
		return new Version(Convert.ToInt32(version.Substring(0, indexFirstDot)), Convert.ToInt32(version.SubstringSpecial(indexFirstDot + 1, indexSecondDot)), Convert.ToInt32(StringHelperClass.SubstringSpecial(version, indexSecondDot + 1, versionEnd)), snapshot);
	  }
