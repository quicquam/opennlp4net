...
namespace opennlp.tools.namefind
{
	...
	public class TokenNameFinderCrossValidator
	{
	  ...
	  private class NameToDocumentSampleStream : FilterObjectStream<NameSample, DocumentSample>
	  {
		...
		protected internal NameToDocumentSampleStream(TokenNameFinderCrossValidator outerInstance, ObjectStream<NameSample> samples) : base(samples)
		{
			this.outerInstance = outerInstance;
		}
...
//ORIGINAL LINE: @Override public void reset() throws java.io.IOException, UnsupportedOperationException
	  ...
	  private class DocumentToNameSampleStream : FilterObjectStream<DocumentSample, NameSample>
	  {
		...
		protected internal DocumentToNameSampleStream(TokenNameFinderCrossValidator outerInstance, ObjectStream<DocumentSample> samples) : base(samples)
		{
			this.outerInstance = outerInstance;
		}
...
//JAVA TO C# CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	  ...
	  [Obsolete("use <seealso cref=\"#TokenNameFinderCrossValidator(String, String, opennlp.tools.util.TrainingParameters, byte[] , java.util.Map, TokenNameFinderEvaluationMonitor...)\"/>")]
	  public TokenNameFinderCrossValidator(string languageCode, int cutoff, int iterations) : this(languageCode, null, cutoff, iterations)
	  {
	  }
	  ...
	  /// instead and pass in a TrainingParameters object. 
	  [Obsolete("use <seealso cref=\"#TokenNameFinderCrossValidator(String, String, opennlp.tools.util.TrainingParameters, byte[] , java.util.Map, TokenNameFinderEvaluationMonitor...)\"/>")]
	  public TokenNameFinderCrossValidator(string languageCode, string type, sbyte[] featureGeneratorBytes, IDictionary<string, object> resources, int iterations, int cutoff)
	  {
		this.languageCode = languageCode;
		this.type = type;
	  ...
	  /// <param name="resources">
	  ///          the resources for the name finder or null if none </param>
	  public TokenNameFinderCrossValidator(string languageCode, string type, TrainingParameters trainParams, sbyte[] featureGeneratorBytes, IDictionary<string, object> resources, params TokenNameFinderEvaluationMonitor[] listeners)
	  {
...
//ORIGINAL LINE: public void evaluate(opennlp.tools.util.ObjectStream<NameSample> samples, int nFolds) throws java.io.IOException
	  public virtual void evaluate(ObjectStream<NameSample> samples, int nFolds)
	  {
		...
		CrossValidationPartitioner<DocumentSample> partitioner = new CrossValidationPartitioner<DocumentSample>(new NameToDocumentSampleStream(this, samples), nFolds);

		while (partitioner.hasNext())
		{

            CrossValidationPartitioner<DocumentSample>.TrainingSampleStream trainingSampleStream = partitioner.next();

		  TokenNameFinderModel model = opennlp.tools.namefind.NameFinderME.train(languageCode, type, new DocumentToNameSampleStream(this, trainingSampleStream), @params, featureGeneratorBytes, resources);
