...
namespace opennlp.tools.namefind
{
	...
	public class TokenNameFinderEvaluator : Evaluator<NameSample>
	{
	  ...
	  public TokenNameFinderEvaluator(TokenNameFinder nameFinder, params TokenNameFinderEvaluationMonitor[] listeners) : base(listeners)
	  {
...
//ORIGINAL LINE: final NameSampleDataStream sampleStream = new NameSampleDataStream(new opennlp.tools.util.PlainTextByLineStream(new java.io.InputStreamReader(new java.io.FileInputStream(args[2]), args[1])));
		  NameSampleDataStream sampleStream = new NameSampleDataStream(new PlainTextByLineStream(new InputStreamReader(new FileInputStream(args[2]), args[1])));
...
//ORIGINAL LINE: final opennlp.tools.cmdline.PerformanceMonitor monitor = new opennlp.tools.cmdline.PerformanceMonitor("sent");
	    ...
	    private class ObjectStreamAnonymousInnerClassHelper : ObjectStream<NameSample>
	  {
		  ...
		  public ObjectStreamAnonymousInnerClassHelper(opennlp.tools.namefind.NameSampleDataStream sampleStream, PerformanceMonitor monitor)
		  {
