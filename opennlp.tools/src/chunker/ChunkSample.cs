...
namespace opennlp.tools.chunker
{
	...
	public class ChunkSample
	{
	  ...
	  public virtual Span[] PhrasesAsSpanList
	  {
		  get
		  {
			return phrasesAsSpanList(Sentence, Tags, Preds);
		  }
	  }
	  ...
	  public static Span[] phrasesAsSpanList(string[] aSentence, string[] aTags, string[] aPreds)
	  {
		...
		for (int ci = 0, cn = aPreds.Length; ci < cn; ci++)
		{
		  ...
		  if (pred.StartsWith("B-", StringComparison.Ordinal) || (!pred.Equals("I-" + startTag) && !pred.Equals("O"))) // start
		  {
...
//ORIGINAL LINE: private static void validateArguments(int sentenceSize, int tagsSize, int predsSize) throws IllegalArgumentException
	  private static void validateArguments(int sentenceSize, int tagsSize, int predsSize)
	  {
		...
		{
		  throw new System.ArgumentException("All arrays must have the same length: " + "sentenceSize: " + sentenceSize + ", tagsSize: " + tagsSize + ", predsSize: " + predsSize + "!");
		}
	  }
	  ...
	  public override bool Equals(object obj)
	  {
		...
		else if (obj is ChunkSample)
		{
		  ...
		  return Arrays.Equals(Sentence, a.Sentence) && Arrays.Equals(Tags, a.Tags) && Arrays.Equals(Preds, a.Preds);
		}
		else
		{
		  return false;
