...
namespace opennlp.tools.postag
{
    ...
    public class POSTaggerFactory : BaseToolFactory
    {
        ...
        {
            get { return new DefaultPOSContextGenerator(0, Dictionary); }
        }
        ...
        {
            get { return new DefaultPOSSequenceValidator(TagDictionary); }
        }
                ...
                if (tagdictEntry is POSDictionary)
                {
                    if (!this.artifactProvider.LoadedFromSerialized)
                    {
                        AbstractModel posModel =
                            this.artifactProvider.getArtifact<AbstractModel>(POSModel.POS_MODEL_ENTRY_NAME);
