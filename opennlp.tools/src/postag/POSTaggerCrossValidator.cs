...
namespace opennlp.tools.postag
{
	...
	public class POSTaggerCrossValidator
	{
	  ...
	  /// </summary>
	  public POSTaggerCrossValidator(string languageCode, TrainingParameters trainParam, Jfile tagDictionary, int? ngramCutoff, int? tagdicCutoff, string factoryClass, params POSTaggerEvaluationMonitor[] listeners)
	  {
	  ...
	  /// </summary>
	  public POSTaggerCrossValidator(string languageCode, TrainingParameters trainParam, POSTaggerFactory factory, params POSTaggerEvaluationMonitor[] listeners)
	  {
		this.languageCode = languageCode;
		this.@params = trainParam;
		this.listeners = listeners;
	  ...
	  ///             <seealso cref="POSTaggerFactory"/>. 
	  public POSTaggerCrossValidator(string languageCode, ModelType modelType, POSDictionary tagDictionary, Dictionary ngramDictionary, int cutoff, int iterations) : this(languageCode, create(modelType, cutoff, iterations), create(ngramDictionary, tagDictionary))
	  {
	  }
	  ...
	  ///             instead and pass in a <seealso cref="TrainingParameters"/> object and a
	  ///             <seealso cref="POSTaggerFactory"/>. 
	  public POSTaggerCrossValidator(string languageCode, ModelType modelType, POSDictionary tagDictionary, Dictionary ngramDictionary) : this(languageCode, create(modelType, 5, 100), create(ngramDictionary, tagDictionary))
	  {
	  }
	  ...
	  public POSTaggerCrossValidator(string languageCode, TrainingParameters trainParam, POSDictionary tagDictionary, params POSTaggerEvaluationMonitor[] listeners) : this(languageCode, trainParam, create(null, tagDictionary), listeners)
	  {
	  }
	  ...
	  ///             sub-class. 
	  public POSTaggerCrossValidator(string languageCode, TrainingParameters trainParam, POSDictionary tagDictionary, int? ngramCutoff, params POSTaggerEvaluationMonitor[] listeners) : this(languageCode, trainParam, create(null, tagDictionary), listeners)
	  {
		this.ngramCutoff = ngramCutoff;
	  }
	  ...
	  ///             <seealso cref="#POSTaggerCrossValidator(String, TrainingParameters, POSTaggerFactory, POSTaggerEvaluationMonitor...)"/>
	  ///             instead and pass in a <seealso cref="POSTaggerFactory"/>. 
	  public POSTaggerCrossValidator(string languageCode, TrainingParameters trainParam, POSDictionary tagDictionary, Dictionary ngramDictionary, params POSTaggerEvaluationMonitor[] listeners) : this(languageCode, trainParam, create(ngramDictionary, tagDictionary), listeners)
	  {
	  }
	  ...
	  public virtual void evaluate(ObjectStream<POSSample> samples, int nFolds)
	  {
		...
		while (partitioner.hasNext())
		{
		  ...
		  if (this.tagdicCutoff != null)
		  {
			...
			{
			  POSTaggerME.populatePOSDictionary(trainingSampleStream, (MutableTagDictionary)dict, this.tagdicCutoff.Value);
			}
			else
			{
			  throw new System.ArgumentException("Can't extend a TagDictionary that does not implement MutableTagDictionary.");
			}
			trainingSampleStream.reset();
		  }
