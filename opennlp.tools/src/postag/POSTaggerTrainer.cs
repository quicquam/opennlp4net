...
namespace opennlp.tools.postag
{
    ...
    [Obsolete(
        "Use <seealso cref=\"POSTaggerME#train(String, ObjectStream, opennlp.tools.util.model.ModelType, POSDictionary, Dictionary, int, int)\"/> instead."
        )]
    public class POSTaggerTrainer
    {
        ...
        private static void usage()
        {
            Console.Error.WriteLine(
                "Usage: POSTaggerTrainer [-encoding encoding] [-dict dict_file] -model [perceptron,maxnet] training_data model_file_name [cutoff] [iterations]");
            Console.Error.WriteLine(
                "This trains a new model on the specified training file and writes the trained model to the model file.");
            Console.Error.WriteLine("-encoding Specifies the encoding of the training file");
            Console.Error.WriteLine(
                "-dict Specifies that a dictionary file should be created for use in distinguising between rare and non-rare words");
            ...
            Dictionary ngramDictionary, int cutoff, int iterations)
        {
            GISModel posModel = opennlp.maxent.GIS.trainModel(iterations,
                new TwoPassDataIndexer(
                    new POSSampleEventStream(samples, new DefaultPOSContextGenerator(ngramDictionary)), cutoff));
            ...
            try
            {
                ...
                if (sequence)
                {
                    ...
                    if (encoding == null)
                    {
                        ...
                        {
                            ss =
                                new POSSampleSequenceStream(
                                    new WordTagSampleStream(new InputStreamReader(new FileInputStream(inFile))));
                        }
                        else
                        {
                            POSContextGenerator cg =
                                new DefaultPOSContextGenerator(new Dictionary(new FileInputStream(dict)));

                            ss =
                                new POSSampleSequenceStream(
                                    new WordTagSampleStream((new InputStreamReader(new FileInputStream(inFile)))), cg);
                        }
                    }
                    else
                    {
