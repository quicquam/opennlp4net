...
namespace opennlp.tools.postag
{
	...
	public sealed class POSModel : BaseModel
	{
	  ...
	  ///             instead. 
	  public POSModel(string languageCode, AbstractModel posModel, POSDictionary tagDictionary, Dictionary ngramDict, IDictionary<string, string> manifestInfoEntries) : this(languageCode, posModel, manifestInfoEntries, new POSTaggerFactory(ngramDict, tagDictionary))
	  {
	  ...
	  ///             <seealso cref="#POSModel(String, AbstractModel, Map, POSTaggerFactory)"/>
	  ///             instead. 
	  public POSModel(string languageCode, AbstractModel posModel, POSDictionary tagDictionary, Dictionary ngramDict) : this(languageCode, posModel, null, new POSTaggerFactory(ngramDict, tagDictionary))
	  {
	  }

	  public POSModel(string languageCode, AbstractModel posModel, IDictionary<string, string> manifestInfoEntries, POSTaggerFactory posFactory) : base(COMPONENT_NAME, languageCode, manifestInfoEntries, posFactory)
	  {
...
//ORIGINAL LINE: @Override protected void validateArtifactMap() throws opennlp.tools.util.InvalidFormatException
	  ...
	  public AbstractModel PosModel
	  {
		  get
		  {
			return (AbstractModel) artifactMap[POS_MODEL_ENTRY_NAME];
		  }
	  }
