...
namespace opennlp.tools.formats.convert
{
	...
	public class ParseToTokenSampleStreamFactory : DetokenizerSampleStreamFactory<TokenSample>
	{
	  ...
	  public override ObjectStream<TokenSample> create(string[] args)
	  {
		...
		ObjectStream<Parse> parseSampleStream = StreamFactoryRegistry.getFactory(typeof(Parse), StreamFactoryRegistry.DEFAULT_FORMAT).create(ArgumentParser.filter(args, typeof(WordTagSampleStreamFactory.Parameters)));
