...
namespace opennlp.tools.coref.sim
{
	...
	public class Context : Mention
	{
	  ...
	  public Context(Span span, Span headSpan, int entityId, Parse parse, string extentType, string nameType, HeadFinder headFinder) : base(span,headSpan,entityId,parse,extentType,nameType)
	  {
		init(headFinder);
	  }
	  ...
	  private void init(HeadFinder headFinder)
	  {
		...
		if (headTokenTag.StartsWith("NN", StringComparison.Ordinal) && !headTokenTag.StartsWith("NNP", StringComparison.Ordinal))
		{
		  this.synsets = getSynsetSet(this);
		}
		else
