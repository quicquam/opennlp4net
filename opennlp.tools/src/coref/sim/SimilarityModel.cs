...
namespace opennlp.tools.coref.sim
{
    ...
    public class SimilarityModel : TestSimilarityModel, TrainSimilarityModel
    {
	  ...
	  */
                        ...
                        for (int xi2 = xi1 + 1; xi2 < xl; xi2++)
                        {
                            ...
                            do
                            {
                                ...
                                if (!exclusionSet.Contains(sec1))
                                {
                                    if (debugOn)
                                    {
                                        Console.Error.WriteLine(ec1.ToString() + " " + entityNameSet + " " +
                                                                sec1.ToString() + " " + nameSets[sec1.Id]);
                                    }
        ...
        {
            return !np.HeadTokenTag.StartsWith("NNP", StringComparison.Ordinal) &&
                   np.HeadTokenTag.StartsWith("NN", StringComparison.Ordinal);
        }
        ...
        public static void Main(string[] args)
        {
            ...
            for (string line = @in.readLine(); line != null; line = @in.readLine())
            {
                ...
                Console.WriteLine(p + " " +
                                  model.getFeatures(Context.parseContext(words[0]), Context.parseContext(words[1])));
            }
        }
    }
}