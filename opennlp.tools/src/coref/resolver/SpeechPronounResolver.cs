...
//ORIGINAL LINE: public SpeechPronounResolver(String projectName, ResolverMode m, NonReferentialResolver nrr) throws java.io.IOException
	  public SpeechPronounResolver(string projectName, ResolverMode m, NonReferentialResolver nrr) : base(projectName,"fmodel", m, 30,nrr)
	  {
		showExclusions = false;
		preferFirstReferent = true;
	  }
	  ...
	  protected internal override IList<string> getFeatures(MentionContext mention, DiscourseEntity entity)
	  {
		...
		if (entity != null)
		{
		  ...
		  if (mention.HeadTokenTag.StartsWith("PRP", StringComparison.Ordinal) && cec.HeadTokenTag.StartsWith("PRP", StringComparison.Ordinal))
		  {
	  ...
	  public override bool canResolve(MentionContext mention)
	  {
		...
		bool fpp = tag != null && tag.StartsWith("PRP", StringComparison.Ordinal) && ResolverUtils.speechPronounPattern.matcher(mention.HeadTokenText).matches();
