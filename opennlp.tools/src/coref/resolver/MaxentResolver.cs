...
namespace opennlp.tools.coref.resolver
{
	...
	public abstract class MaxentResolver : AbstractResolver
	{
	  ...
	  protected internal MaxentResolver(int numberOfEntitiesBack, bool preferFirstReferent) : base(numberOfEntitiesBack)
	  {
...
//ORIGINAL LINE: public MaxentResolver(String modelDirectory, String name, ResolverMode mode, int numberOfEntitiesBack, boolean preferFirstReferent, NonReferentialResolver nonReferentialResolver) throws java.io.IOException
	  public MaxentResolver(string modelDirectory, string name, ResolverMode mode, int numberOfEntitiesBack, bool preferFirstReferent, NonReferentialResolver nonReferentialResolver) : base(numberOfEntitiesBack)
	  {
		this.preferFirstReferent = preferFirstReferent;
...
//ORIGINAL LINE: public MaxentResolver(String modelDirectory, String modelName, ResolverMode mode, int numberEntitiesBack) throws java.io.IOException
	  public MaxentResolver(string modelDirectory, string modelName, ResolverMode mode, int numberEntitiesBack) : this(modelDirectory, modelName, mode, numberEntitiesBack, false)
	  {
	  }
...
//ORIGINAL LINE: public MaxentResolver(String modelDirectory, String modelName, ResolverMode mode, int numberEntitiesBack, NonReferentialResolver nonReferentialResolver) throws java.io.IOException
	  public MaxentResolver(string modelDirectory, string modelName, ResolverMode mode, int numberEntitiesBack, NonReferentialResolver nonReferentialResolver) : this(modelDirectory, modelName, mode, numberEntitiesBack, false,nonReferentialResolver)
	  {
	  }
...
//ORIGINAL LINE: public MaxentResolver(String modelDirectory, String modelName, ResolverMode mode, int numberEntitiesBack, boolean preferFirstReferent) throws java.io.IOException
	  public MaxentResolver(string modelDirectory, string modelName, ResolverMode mode, int numberEntitiesBack, bool preferFirstReferent) : this(modelDirectory, modelName, mode, numberEntitiesBack, preferFirstReferent, new DefaultNonReferentialResolver(modelDirectory, modelName, mode))
	  {
...
//ORIGINAL LINE: public MaxentResolver(String modelDirectory, String modelName, ResolverMode mode, int numberEntitiesBack, boolean preferFirstReferent, double nonReferentialProbability) throws java.io.IOException
	  public MaxentResolver(string modelDirectory, string modelName, ResolverMode mode, int numberEntitiesBack, bool preferFirstReferent, double nonReferentialProbability) : this(modelDirectory, modelName, mode, numberEntitiesBack, preferFirstReferent, new FixedNonReferentialResolver(nonReferentialProbability))
	  {
		//this(projectName, modelName, mode, numberEntitiesBack, preferFirstReferent, SingletonNonReferentialResolver.getInstance(projectName,mode));
	  }
	  ...
	  public override DiscourseEntity resolve(MentionContext ec, DiscourseModel dm)
	  {
		...
		for (; ei < getNumEntities(dm); ei++)
		{
		  ...
		  if (excluded(ec, de))
		  {
			...
			if (debugOn)
			{
			  Console.Error.WriteLine("excluded " + this + ".resolve: " + ec.toText() + " -> " + de + " " + candProbs[ei]);
			}
		  }
		  else
		  {
			...
			if (debugOn)
			{
			  Console.Error.WriteLine(this + ".resolve: " + ec.toText() + " -> " + de + " (" + ec.Gender + "," + de.Gender + ") " + candProbs[ei] + " " + lfeatures);
			}
		  }
		  ...
		  for (int ei = 0; ei < getNumEntities(dm); ei++)
		  {
			...
			else
			{
			  ...
			  //if (!sampleSelection || (mention.getId() != -1 && entityMention.getId() == mention.getId()) || (!nonReferentFound && useAsDifferentExample)) {
				...
				if (debugOn)
				{
				  Console.Error.WriteLine(this + ".retain: " + mention.Id + " " + mention.toText() + " -> " + entityMention.Id + " " + cde);
				}
				if (mention.Id != -1 && entityMention.Id == mention.Id)
				{
				  referentFound = true;
