...
namespace opennlp.tools.coref
{
	...
	public abstract class AbstractLinker : Linker
	{
	  ...
	  protected internal virtual void updateExtent(DiscourseModel dm, MentionContext mention, DiscourseEntity entity, bool useDiscourseModel)
	  {
		if (useDiscourseModel)
		{
		  ...
		  else
		  {
			...
			entity = new DiscourseEntity(mention, mention.Gender, mention.GenderProb, mention.Number, mention.NumberProb);
			dm.addEntity(entity);
		  }
		}
		else
		{
		  if (entity != null)
		  {
			DiscourseEntity newEntity = new DiscourseEntity(mention, mention.Gender, mention.GenderProb, mention.Number, mention.NumberProb);
...
//ORIGINAL LINE: public void train() throws java.io.IOException
	  ...
	  public virtual MentionContext[] constructMentionContexts(Mention[] mentions)
	  {
		...
		for (int mi = 0,mn = mentions.Length;mi < mn;mi++)
		{
		  ...
		  contexts[mi] = new MentionContext(mentions[mi], mentionInSentenceIndex, numMentionsInSentence, mi, sentenceIndex, HeadFinder);
