...
namespace opennlp.tools.tokenize
{
    ...
    public class TokSpanEventStream : AbstractEventStream<TokenSample>
    {
        ...
        /// <param name="cg"> </param>
        public TokSpanEventStream(ObjectStream<TokenSample> tokenSamples, bool skipAlphaNumerics, Pattern alphaNumeric,
            TokenContextGenerator cg) : base(tokenSamples)
        {
        ...
        /// <param name="skipAlphaNumerics"> </param>
        /// <param name="cg"> </param>
        public TokSpanEventStream(ObjectStream<TokenSample> tokenSamples, bool skipAlphaNumerics,
            TokenContextGenerator cg) : base(tokenSamples)
        {
                    ...
                    if (ctok.Length > 1 && (!skipAlphaNumerics || !alphaNumeric.matcher(ctok).matches()))
                    {
                        ...
                        for (int ti = lastTrainingToken + 1; ti < tokens.Length; ti++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    logger.warning("Bad training token: " + tokens[ti] + " cand: " + cSpan + " token=" +
                                                   StringHelperClass.SubstringSpecial(text, tokens[ti].Start,
                                                       tokens[ti].End));
                                }
                            }
                        }
