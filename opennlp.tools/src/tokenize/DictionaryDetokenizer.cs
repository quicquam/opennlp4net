...
namespace opennlp.tools.tokenize
{
    ...
    public class DictionaryDetokenizer : Detokenizer
    {
        ...
        public virtual string detokenize(string[] tokens, string splitMarker)
        {
            ...
            {
                throw new System.ArgumentException("tokens and operations array must have same length: tokens=" +
            ...
            for (int i = 0; i < tokens.Length; i++)
            {
                ...
                else if (operations[i + 1].Equals(Detokenizer_DetokenizationOperation.MERGE_TO_LEFT) ||
                         operations[i + 1].Equals(Detokenizer_DetokenizationOperation.MERGE_BOTH))
                {
                    isAppendSpace = false;
                    isAppendSplitMarker = true;
                }
                    // if this token is move right, no space 
                else if (operations[i].Equals(Detokenizer_DetokenizationOperation.MERGE_TO_RIGHT) ||
                         operations[i].Equals(Detokenizer_DetokenizationOperation.MERGE_BOTH))
                {
                    isAppendSpace = false;
                    isAppendSplitMarker = true;
                }
