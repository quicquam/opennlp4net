...
//ORIGINAL LINE: protected abstract Parse[] advanceParses(final Parse p, double probMass);
	  ...
	  public virtual Parse[] parse(Parse tokens, int numParses)
	  {
		...
		while (odh.size() > 0 && (completeParses.size() < M || (odh.first()).Prob < minComplete) && derivationStage < maxDerivationLength)
		{
		  ndh = new ListHeap<Parse>(K);
...
//JAVA TO C# CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
		  for (IEnumerator<Parse> pi = odh.GetEnumerator(); pi.hasNext() && derivationRank < K; derivationRank++) // forearch derivation
		  {
			...
			else
			{
			  ...
			  {
				Console.Error.WriteLine("Couldn't advance parse " + derivationStage + " stage " + derivationRank + "!\n");
			  }
			  advanceTop(tp);
	  ...
	  protected internal virtual Parse[] advanceChunks(Parse p, double minChunkScore)
	  {
		...
		for (int si = 0, sl = cs.Length; si < sl; si++)
		{
		  ...
		  for (int j = 0; j <= tags.Length; j++)
		  {
			...
			if (j != tags.Length && tags[j].StartsWith(CONT, StringComparison.Ordinal)) // if continue just update end chunking tag don't use contTypeMap
			{
			  end = j;
			}
			else //make previous constituent if it exists
			{
			  if (type != null)
			  {
				...
				Parse chunk = new Parse(p1.Text, new Span(p1.Span.Start, p2.Span.End), type, 1, headRules.getHead(cons, type));
				chunk.isChunk(true);
				newParses[si].insert(chunk);
			  }
			  if (j != tags.Length) //update for new constituent
			  {
				if (tags[j].StartsWith(START, StringComparison.Ordinal)) // don't use startTypeMap these are chunk tags
				{
