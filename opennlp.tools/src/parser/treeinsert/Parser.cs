...
namespace opennlp.tools.parser.treeinsert
{
    ...
    public class Parser : AbstractBottomUpParser
    {
        ...
        public Parser(ParserModel model, int beamSize, double advancePercentage)
            : this(
                model.BuildModel, model.AttachModel, model.CheckModel, new POSTaggerME(model.ParserTaggerModel),
                new ChunkerME(model.ParserChunkerModel, ChunkerME.DEFAULT_BEAM_SIZE,
                    new ParserChunkerSequenceValidator(model.ParserChunkerModel),
                    new ChunkContextGenerator(ChunkerME.DEFAULT_BEAM_SIZE)), model.HeadRules, beamSize,
                advancePercentage)
        {
        }
        ...
        [Obsolete]
        public Parser(AbstractModel buildModel, AbstractModel attachModel, AbstractModel checkModel, POSTagger tagger,
            Chunker chunker, HeadRules headRules, int beamSize, double advancePercentage)
        ...
        [Obsolete]
        public Parser(AbstractModel buildModel, AbstractModel attachModel, AbstractModel checkModel, POSTagger tagger,
            Chunker chunker, HeadRules headRules)
            : this(
                buildModel, attachModel, checkModel, tagger, chunker, headRules, defaultBeamSize,
        ...
        protected internal override Parse[] advanceParses(Parse p, double probMass)
        {
            ...
            if (debugOn)
            {
                Console.WriteLine("adi=" + advanceNodeIndex + " " + advanceNode.Type + "." + advanceNode.Label + " " +
                ...
                else
                {
                    ...
                    for (int fi = 0, fs = rf.Count; fi < fs; fi++)
                    {
                        ...
                        if (debugOn)
                        {
                            ...
                            Console.WriteLine("Frontier node(" + fi + "): " + fn.Type + "." + fn.Label + " " + fn +
                                              " <- " + advanceNode.Type + " " + advanceNode + " d=" +
                        ...
                        for (int ai = 0; ai < attachments.Length; ai++)
                        {
                            ...
                            if (prob > q &&
                                ((!checkComplete && (attachments[ai] != daughterAttachIndex || !isComplete(fn))) ||
                                 (checkComplete &&
                                  ((attachments[ai] == daughterAttachIndex && !isComplete(fn)) ||
                            ...
                            else
                            {
                                if (debugOn)
                                {
                                    Console.WriteLine("Skipping " + fn.Type + "." + fn.Label + " " + fn + " daughter=" +
                                                      (attachments[ai] == daughterAttachIndex) + " complete=" +
                                                      isComplete(fn) + " prob=" + prob);
                                }
                            }
                        }
                        if (checkComplete && !isComplete(fn))
                        {
                            if (debugOn)
                            {
                                Console.WriteLine("Stopping at incomplete node(" + fi + "): " + fn.Type + "." + fn.Label +
                                                  " " + fn);
                            }
                            break;
                        }
            ...
            TrainingParameters mlParams)
        {
            ...
            // chunk
            ChunkerModel chunkModel = ChunkerME.train(languageCode, new ChunkSampleStream(parseSamples),
