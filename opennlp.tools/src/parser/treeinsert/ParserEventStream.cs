...
namespace opennlp.tools.parser.treeinsert
{
	...
	public class ParserEventStream : AbstractParserEventStream
	{
	  ...
	  public ParserEventStream(ObjectStream<Parse> d, HeadRules rules, ParserEventTypeEnum etype, Dictionary dict) : base(d, rules, etype, dict)
	  {
	  }
	  ...
	  public ParserEventStream(ObjectStream<Parse> d, HeadRules rules, ParserEventTypeEnum etype) : base(d, rules, etype)
	  {
	  }
	  ...
	  */
	  ...
	  protected internal override void addParseEvents(IList<Event> parseEvents, Parse[] chunks)
	  {
		...
		for (int ci = 0;ci < chunks.Length;ci++)
		{
		  ...
		  if (ci == 0)
		  {
			Parse top = new Parse(currentChunks[ci].Text,new Span(0,currentChunks[ci].Text.Length),AbstractBottomUpParser.TOP_NODE,1,0);
		  ...
		  else
		  {
			...
			{
			  Console.Error.WriteLine("fontiers mis-aligned: " + currentRightFrontier.Count + " != " + rightFrontier.Count + " " + currentRightFrontier + " " + rightFrontier);
			  Environment.Exit(1);
			}
			...
			for (int cfi = 0;cfi < currentRightFrontier.Count;cfi++)
			{
			  ...
			  if (!Parser.checkComplete || !Parser.COMPLETE.Equals(cfn.Label))
			  {
				...
				if (debug)
				{
					Console.Error.WriteLine("Looking at attachment site (" + cfi + "): " + cfn.Type + " ci=" + i + " cs=" + nonPunctChildCount(cfn) + ", " + cfn + " :for " + currentChunks[ci].Type + " " + currentChunks[ci] + " -> " + parents);
				}
			  ...
			  else
			  {
				if (debug)
				{
					Console.Error.WriteLine("Skipping (" + cfi + "): " + cfn.Type + "," + cfn.PreviousPunctuationSet + " " + cfn + " :for " + currentChunks[ci].Type + " " + currentChunks[ci] + " -> " + parents);
				}
			  }
			...
			for (int cfi = 0;cfi < currentRightFrontier.Count;cfi++)
			{
			  Parse frontierNode = rightFrontier[cfi];
			  Parse cfn = currentRightFrontier[cfi];
			  if (attachNode == null && parents.ContainsKey(frontierNode.Parent) && frontierNode.Type.Equals(frontierNode.Parent.Type)) //&& frontierNode.getParent().getLabel() == null) {
			  {
			...
			if (attachNode != null)
			{
			  if (attachType == Parser.ATTACH_DAUGHTER)
			  {
				...
				if (debug)
				{
					Console.Error.WriteLine("daughter attach a=" + attachNode.Type + ":" + attachNode + " d=" + daughter + " com=" + lastChild(chunks[ci], rightFrontier[attachNodeIndex]));
				}
			  ...
			  else if (attachType == Parser.ATTACH_SISTER)
			  {
				...
				if (debug)
				{
					Console.Error.WriteLine("sister attach a=" + attachNode.Type + ":" + attachNode + " s=" + sister + " ap=" + attachNode.Parent + " com=" + lastChild(chunks[ci], rightFrontier[attachNodeIndex]));
				}
