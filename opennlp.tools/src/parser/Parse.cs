...
//ORIGINAL LINE: public void insert(final Parse constituent)
	  ...
	  public virtual Parse adjoinRoot(Parse node, HeadRules rules, int parseIndex)
	  {
		...
		Parse adjNode = new Parse(this.text,new Span(lastChild.Span.Start,node.Span.End),lastChild.Type,1,rules.getHead(new Parse[]{lastChild,node},lastChild.Type));
	  ...
	  public virtual Parse adjoin(Parse sister, HeadRules rules)
	  {
		...
		Parse adjNode = new Parse(this.text,new Span(lastChild.Span.Start,sister.Span.End),lastChild.Type,1,rules.getHead(new Parse[]{lastChild,sister},lastChild.Type));
		adjNode.parts.Add(lastChild);
	  ...
	  public virtual bool PosTag
	  {
		  get
		  {
			return (parts.Count == 1 && (parts[0]).Type.Equals(AbstractBottomUpParser.TOK_NODE));
		  }
	  }
	...
	//    result = 37*result + label.hashCode();
	  ...
	  private void codeTree(Parse p, int[] levels)
	  {
		...
		for (int ki = 0;ki < kids.Length;ki++)
		{
		  ...
		  Console.WriteLine(levelsBuff.ToString() + ki + "] " + kids[ki].Type + " " + kids[ki].GetHashCode() + " -> " + kids[ki].Parent.GetHashCode() + " " + kids[ki].Parent.Type + " " + kids[ki].CoveredText);
			...
			else
			{
			  ...
			  else
			  {
				if (commonParent.Type.Equals("NP"))
				{
				  ...
				  {
					commonParent.insert(new Parse(commonParent.Text,commonParent.Span,tag,1.0,commonParent.HeadIndex));
				  }
				}
			  }
			}
